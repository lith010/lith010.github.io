...
a.type="text/css";a.rel="stylesheet";a.href="$services.webjars.url('org.xwiki.platform:xwiki-platform-tree-webjar', 'tree.min.css', {'evaluate': true})";document.getElementsByTagName("head")[0].appendChild(a);require(["xwiki-tree"],this.initXTree.bind(this))},initXTree:function(a){this.node.removeClassName("loading");this.node.update();this.node.empty()&&this.node.insert((new Element("h4",{"class":"legend"})).update(d.availableDocuments));this.container=new Element("div",{id:"packageDescription"});
this.node.insert(this.container);this.container.insert(this.createPackageHeader(this.infos));var b=(new Element("span")).update(d.none);b.observe("click",this.onIgnoreAllDocuments.bind(this));var c=(new Element("span")).update(d.all);c.observe("click",this.onRestoreAllDocuments.bind(this));this.container.insert((new Element("div",{"class":"selectLinks"})).insert(d.select).insert(b).insert(", ").insert(c));this.list=new Element("ul");this.container.insert((new Element("div",{id:"package","class":"package xtree jstree-no-links"})).update(this.list));
Object.values(this.entities.children).each(this.addSpace.bind(this));this.container.insert(this.createPackageFormSubmit(this.infos));a("#package").xtree({plugins:["checkbox"]});this.xtree=a.jstree.reference(a("#package"));this.xtree.check_all()},onIgnoreAllDocuments:function(){this.xtree.uncheck_all()},onRestoreAllDocuments:function(){this.xtree.check_all()},onPackageInfosRequestFailed:function(a){this.node.update();a="Failed to retrieve package information. Reason: "+(a.statusText||"Server not responding");
...
{type:"radio",name:"historyStrategy",value:"replace"})).insert(d.replaceDocumentHistory));b.insert((new Element("div",{"class":"historyStrategyOption"})).insert(new Element("input",{type:"radio",name:"historyStrategy",value:"reset"})).insert(d.resetHistory));f.hasBackupPackImportRights&&(c=new Element("input",{type:"checkbox",name:"importAsBackup",value:"true"}),a.backup&&(c.checked=!0),b.insert((new Element("div",{"class":"importOption"})).insert(c).insert(d.importAsBackup)));a=new Element("span",
{"class":"buttonwrapper"});c=new Element("input",{type:"submit",value:d["import"],"class":"button"});c.observe("click",this.onPackageSubmit.bind(this));a.insert(c);b.insert(a);return b},onPackageSubmit:function(){var a=this.xtree.get_bottom_checked(!0);if(0==a.length)a=(new Element("span",{"class":"warningmessage"})).update(d.selectionEmpty),$("packagecontainer").down("div.packagesubmit span.warningmessage")||($("packagecontainer").select("div.packagesubmit input").last().insert({after:a}),Element.remove.delay(5,
a));else{var b={action:"import"};b.name=this.name;b.historyStrategy=$("packageDescription").down("input[type=radio][value='add']").checked?"add":$("packageDescription").down("input[type=radio][value='replace']").checked?"replace":"reset";f.hasBackupPackImportRights&&(b.importAsBackup=$("packageDescription").down("input[type=checkbox][name='importAsBackup']").checked?"true":"false");b.ajax="1";var c=[];a.each(function(e){var g=e.data.reference+":"+e.data.locale;c.push(g);b["language_"+g]=e.data.locale});
b.pages=c;this.node.update();this.node.addClassName("loading");this.node.setStyle("min-height:200px");new Ajax.Request(f.currentDocument.getURL("import",Object.toQueryString(window.location.href.parseQuery())),{method:"post",parameters:b,onSuccess:function(e){$("packagecontainer").removeClassName("loading");$("packagecontainer").update(e.responseText)},onFailure:function(e){e="Failed to import documents. Reason: "+(e.statusText||"Server not responding");$("packagecontainer").removeClassName("loading");
$("packagecontainer").update((new Element("div",{"class":"errormessage"})).update(e))}})}},createPackageHeader:function(a){var b=new Element("div",{"class":"packageinfos"});b.insert((new Element("div")).insert((new Element("span",{"class":"label"})).update(d["package"])).insert((new Element("span",{"class":"filename"})).update(this.name)));""!==a.name&&b.insert((new Element("div")).insert((new Element("span",{"class":"label"})).update(d.description)).insert((new Element("span",{"class":"name"})).update(a.name)));
