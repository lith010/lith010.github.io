...
(a.style.cursor="pointer",a.title="$escapetool.javascript($services.localization.render('platform.core.rendering.error.readTechnicalInformation'))",Event.observe(a,"click",function(e){e.element().closest(".xwikirenderingerror").next(".xwikirenderingerrordescription").toggleClassName("hidden")}))})},fixLinksTargetAttribute:function(d){d=$(d||"body").select("a[rel]");for(var a=0;a<d.length;a++){var c=d[a];if(!c.up(".xRichTextEditor")&&c.getAttribute("href")&&c.getAttribute("rel")){for(var e=c.getAttribute("rel").split(" "),
g=0;g<e.length;g++)if("_"==e[g].charAt(0)){c.target=e[g].substring(1);break}else if("external"==e[g]){c.target="_blank";break}c.target&&"_self"!=c.target&&"_parent"!=c.target&&"_top"!=c.target&&(g=function(f,h){for(var k=0;k<f.length;++k)if(f[k]==h)return!0;return!1},g(e,"noopener")||e.push("noopener"),g(e,"noreferrer")||e.push("noreferrer"),c.setAttribute("rel",e.join(" ")))}}},insertSectionEditLinks:function(d){if($xwiki.hasSectionEdit()&&"xwiki/1.0"!=b.docsyntax&&"view"==b.contextaction&&b.hasEdit){var a=
1;d=$(d||"body");if(d="xwikicontent"==d.id?d:d.down("#xwikicontent")){d=d.childNodes;for(var c=new RegExp("H[1-"+$xwiki.getSectionEditingDepth()+"]"),e=0;e<d.length;e++){var g=$(d[e]);if(c.test(g.nodeName)&&!g.classList.contains("wikigeneratedheader")&&!g.down("a.edit_section")){var f=document.createElement("a");f.className="edit_section";f.toggle(g.visible()&&!g.hasClassName("hidden"));f.setAttribute("role","button");f.title=$jsontool.serialize($services.localization.render("edit"));f.innerHTML=
$jsontool.serialize($services.icon.renderHTML("pencil"));var h=document.createElement("span");h.className="sr-only";h.textContent=$jsontool.serialize($services.localization.render("edit"));f.appendChild(h);b.hasRenderer?(h=document.documentElement.getAttribute("data-xwiki-form-token"),f.href=window.docediturl+"?section="+a+"&form_token="+h):(f.className+=" disabled",f.setAttribute("aria-disabled","true"),f.setAttribute("tabindex","-1"),f.title=$jsontool.serialize($services.localization.render("platform.core.rendering.noRendererForSectionEdit")),
f.href="#"+(g.id||""));g.appendChild(f);a++}}}}},insertCreatePageFromTemplateModalBoxes:function(d){if("xwiki/1.0"!=b.docsyntax&&"view"==b.contextaction&&b.hasEdit&&b.widgets.ModalPopup){b.widgets.CreatePagePopup=Class.create(b.widgets.ModalPopup,{initialize:function($super,e){var g=new Element("div",{"class":"modal-popup"});g.insert(e.content);$super(g,{show:{method:this.showDialog,keys:[]},close:{method:this.closeDialog,keys:["Esc"]}},{displayCloseButton:!0,verticalPosition:"center",backgroundColor:"#FFF"});
this.showDialog();this.setClass("createpage-modal-popup")}});d=$(d||"body").select("span.wikicreatelink:not(.skipCreatePagePopup)");for(var a=0;a<d.length;a++)d[a].down("a").observe("click",function(c){new Ajax.Request(c.findElement("a").href.replace(/#.*$/,""),{method:"get",parameters:{xpage:"createinline",ajax:1},onSuccess:function(e){var g=e.getHeader("redirect");if(g)window.location=g;else{var f=new b.widgets.Notification("$escapetool.javascript($services.localization.render('core.create.popup.loading'))",
"inprogress");g=new Element("link",{rel:"stylesheet",type:"text/css",href:'$xwiki.getSkinFile("uicomponents/widgets/select/select.css", true)'});$(document.head).insert(g);require(["$xwiki.getSkinFile('js/xwiki/create.js', true)","$xwiki.getSkinFile('uicomponents/widgets/select/select.js', true)"],function(h){new b.widgets.CreatePagePopup({content:e.responseText});f.hide()})}},onFailure:function(){(new b.widgets.Notification("$escapetool.javascript($services.localization.render('core.create.ajax.error'))",
...
this.fixLinksTargetAttribute(d);this.insertSectionEditLinks(d);this.insertCreatePageFromTemplateModalBoxes(d);this.watchlist.initialize(d);this.registerPanelToggle(d)}});return b}(XWiki||{});document.observe("dom:loaded",XWiki.initialize.bind(XWiki));
