...
k.find(".export-tree");var g=k.data("config").url;f.empty().attr("action",g.replace(/pages=.*?(&|$)/g,""));g=k.data("config").filterHiddenPages;n(e.jstree.reference(b),f,g);k.find('input[type="hidden"][name="filter"]').clone().appendTo(f);f.submit()});require(["xwiki-export-tree","xwiki-export-tree-filter"],function(){e(".export-tree").exportTree()})})}).apply("]]#",$jsontool.serialize([$l10n,$icons]));
