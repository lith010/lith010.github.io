...
#end
#[[*/
'use strict';(function(m,e){function k(){if("inline"==XWiki.contextaction||"edit"==XWiki.contextaction&&$("inline")){var a=$$(".dashboard")[0];a&&require(["scriptaculous/dragdrop"],function(){new XWiki.Dashboard(a)})}return!0}require.config({paths:m});window.XWiki=window.XWiki||{};XWiki.Dashboard=Class.create({initialize:function(a){this.element=a;this.gadgetsClass="XWiki.GadgetClass";this.readMetadata();this.displayWarning();this.edited=!1;this.removed=[];this.element.addClassName("dashboard-edit");
...
(g.statusText||"Server not responding"),"error",{timeout:5}))}.bind(this),on0:function(g){g.request.options.onFailure(g)}.bind(this)})},onEditGadgetClick:function(a){function b(h,n){return h.immediateDescendants().filter(p=>p.hasClassName(n)).first()}var d=a.element().up(".gadget");if(d){var c=b(d,"metadata");if(c){var f=b(c,"isMacro");if(f&&"true"==f.innerHTML){var g=this._getGadgetId(d),l;if(d=b(c,"title"))var q=d.innerHTML;(c=b(c,"content"))&&(l=this._parseMacroCallComment(c.innerHTML));this.runGadgetWizard($(a.target),
{title:q,content:l},function(h){this.onEditGadgetComplete(g,h)}.bind(this))}else(new XWiki.widgets.ModalPopup(e["dashboard.gadget.actions.edit.error.notmacro"],{},{title:e["dashboard.gadget.actions.edit.error.notmacro.title"]})).showDialog()}}},onEditGadgetComplete:function(a,b){var d=this._getMacroCallComment(b.content),c=new Hash;c.set(this.gadgetsClass+"_"+a+"_title",b.title);c.set(this.gadgetsClass+"_"+a+"_content",d);c.set("RequiresHTMLConversion",this.gadgetsClass+"_"+a+"_content");c.set(this.gadgetsClass+
"_"+a+"_content_syntax","xwiki/2.0");c.set("ajax","1");a=this.getFormToken();c.set("form_token",a);this._x_notification=new XWiki.widgets.Notification(e["dashboard.gadget.actions.edit.loading"],"inprogress");new Ajax.Request(this.editURL,{parameters:c,onSuccess:function(f){window.location.reload()}.bind(this),onFailure:function(f){this._x_notification.replace(new XWiki.widgets.Notification(e["dashboard.gadget.actions.edit.failed"]+(f.statusText||"Server not responding"),"error",{timeout:5}))}.bind(this),
on0:function(f){f.request.options.onFailure(f)}.bind(this)})},onRemoveGadget:function(a){var b=a.element(),d=b.up(".gadget");d&&(a=this._getGadgetId(d),this.removed.push(a),new XWiki.widgets.ConfirmedAjaxRequest(this.removeURL,{parameters:{classname:encodeURIComponent(this.gadgetsClass),classid:encodeURIComponent(a),ajax:"1",form_token:this.getFormToken()},onCreate:function(){b.disabled=!0},onSuccess:function(c){d.remove()}.bind(this)},{confirmationText:e["dashboard.gadget.actions.delete.confirm"],
progressMessageText:e["dashboard.gadget.actions.delete.inProgress"],successMessageText:e["dashboard.gadget.actions.delete.done"],failureMessageText:e["dashboard.gadget.actions.delete.failed"]}))},addNewContainerHandler:function(){var a=new Element("div",{"class":"addcontainer",title:e["dashboard.actions.columns.add.tooltip"]});a.update(e["dashboard.actions.columns.add.button"]);a.observe("click",this.onAddColumn.bindAsEventListener(this));this.element.down(".addgadget").insert({before:a})},onAddColumn:function(a){a=
this.containers.last();var b=this._getContainerId(a);b="gadgetcontainer_"+(1+parseInt(b,10));b=new Element("div",{"class":a.readAttribute("class"),id:b});a.removeClassName("last-column");a.insert({after:b});this.containers.push(b);b="container-columns-"+(this.containers.length-1);a=a.parentNode;a.addClassName("container-columns-"+this.containers.length);a.removeClassName(b);var d=[];this.containers.each(function(c){d.push(c.readAttribute("id"))});this.createDragAndDrops()},doEditGadgets:function(a){var b=
this.prepareEditParameters();b.set("ajax","1");var d=this.getFormToken();b.set("form_token",d);new Ajax.Request(this.editURL,{parameters:b,onSuccess:function(c){this.edited=!1;c.responseJSON&&c.responseJSON.newVersion?require(["xwiki-meta"],function(f){f.setVersion(c.responseJSON.newVersion);a&&a()}):a&&a()}.bind(this),onFailure:function(c){this._x_notification=new XWiki.widgets.Notification(e["dashboard.actions.edit.failed"]+(c.statusText||"Server not responding"),"error",{timeout:5});a&&a()}.bind(this),
on0:function(c){c.request.options.onFailure(c)}.bind(this)})},onMoveGadget:function(a){this.edited=!0},saveChanges:function(a){if(this.edited){a.stop();var b=a.element();this._x_edit_notification=new XWiki.widgets.Notification(e["dashboard.actions.save.loading"],"inprogress");this.doEditGadgets(function(){this.edited||b.click();this._x_edit_notification&&this._x_edit_notification.hide()}.bind(this))}},prepareEditParameters:function(){var a=new Hash;this.containers.each(function(b){var d=this._getContainerId(b);
b.select(".gadget").each(function(c,f){c=this._getGadgetId(c);a.set(this.gadgetsClass+"_"+c+"_position",d+", "+(f+1))},this)},this);return a},getFormToken:function(){var a=this.element.up("form");return a&&(a=a.form_token)?a.value:""}});XWiki.domIsLoaded&&k()||document.observe("xwiki:dom:loaded",k)}).apply("]]#",$jsontool.serialize([$paths,$l10n]));
