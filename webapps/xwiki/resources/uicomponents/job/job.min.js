...
function(a,b){var c=f(this);c.find(".ui-progress-background").toggle("NONE"!==a.state);c.find(".ui-progress-message").toggle("NONE"===a.state);"NONE"!==a.state?c.find("#state-none-hint").remove():c.find("#state-none-hint").removeClass("hidden");var d=Math.floor(100*(a.progress.offset||0));c.find(".ui-progress-bar").css("width",d+"%");d=a.log.items||[];d.length&&c.find(".ui-progress-message").html(d[d.length-1].renderedMessage);d=c.find(".ui-question");if("function"===typeof b){var e=a.id.map(encodeURIComponent).join("/");
e=XWiki.contextPath+"/job/wiki/"+p.wiki+"/question/"+e;d.data("answerCallback",b);f.get(e).then(r.bind(d))}else d.empty();t(c,a)},x=function(a){f(this).find(".ui-progress").replaceWith(a.message)},y=function(){};f(".job-status").has(".ui-progress").each(function(){var a=f(this),b=a.attr("data-url");if(""!==b){var c=a.find(".log"),d={createStatusRequest:function(){return{url:b,data:{logOffset:c.find(".log-item").length}}}};a=a.find(".ui-question");a.length&&(a.on("click",".btAnswerConfirm",n),a.on("click",
".btAnswerCancel",n),d.createAnswerRequest=function(e,g){return"function"===typeof g?g():{url:XWiki.contextPath+"/job/question/"+e.map(encodeURIComponent).join("/"),data:g}});(new q(d)).resume().progress(w.bind(this)).then(x.bind(this),y.bind(this))}})});
