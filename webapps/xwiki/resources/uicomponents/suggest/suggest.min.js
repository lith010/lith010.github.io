...
this.fld.cumulativeOffset():this.fld.positionedOffset(),c=this.fld.offsetWidth-2,d=this.options.width||c,e=this.fld.viewportOffset().left,f=$("body").getWidth();a.style.left="left"==this.options.align||"auto"==this.options.align&&e+this.options.width<f?b.left+"px":"center"==this.options.align?b.left+(c-d)/2+"px":b.left+c-d+"px";a.style.top=b.top+this.fld.offsetHeight+this.options.offsety+"px";a.style[this.options.width?"width":"minWidth"]=d+"px";var g=this;a.addEventListener("focusin",()=>g.killTimeout());
a.addEventListener("focusout",()=>g.resetTimeout());a.addEventListener("mouseout",()=>g.clearHighlight());this.resultContainer=new Element("div",{"class":"resultContainer"});a.appendChild(this.resultContainer);$(this.options.parentContainer).insert(a);this.container=a;this.options.insertBeforeSuggestions&&this.resultContainer.insert(this.options.insertBeforeSuggestions);document.fire("xwiki:suggest:containerCreated",{container:this.container,suggest:this})}if(this.isInMultiSourceMode)for(a=0;a<this.sources.length;a++)b=
this.sources[a],b.id=b.id||a,this.resultContainer.down(".results"+b.id)?(this.resultContainer.down(".results"+b.id).down("ul")&&this.resultContainer.down(".results"+b.id).down("ul").remove(),this.options.unifiedLoader?((this.options.loaderNode||this.fld).addClassName("loading"),this.resultContainer.down(".results"+b.id).addClassName("hidden").addClassName("loading")):this.resultContainer.down(".results"+b.id).down(".sourceContent").addClassName("loading")):(c=new Element("div",{"class":"results results"+
...
"undefined"!==typeof b.before&&this.resultContainer.insert(b.before),this.resultContainer.insert(c),"undefined"!==typeof b.after&&this.resultContainer.insert(b.after));else this.resultContainer.down("ul")&&this.resultContainer.down("ul").remove();if("undefined"!==typeof this.options.hideButton&&"object"===typeof this.options.hideButton.positions&&0<this.options.hideButton.positions.length&&!this.container.down(".hide-button"))for(b=this.options.hideButton.positions,a=0;a<b.length;a++)c=(new Element("button",
{"class":"hide-button",type:"button"})).update(this.options.hideButton.text),d={},d[b[a]]=(new Element("div",{"class":"hide-button-wrapper"})).update(c),c.observe("click",this.clearSuggestions.bindAsEventListener(this)),this.container.insert(d);this.container.fire("xwiki:suggest:containerPrepared",{container:this.container,suggest:this});return this.container},createList:function(a,b){this._createList(a,b);this.isInMultiSourceMode&&this.resultContainer.down(".results.loading")||document.fire("xwiki:suggest:updated",
{container:this.container,suggest:this})},_createList:function(a,b){this.isActive=!0;var c=this;this.killTimeout();if(this.isInMultiSourceMode){var d=this.resultContainer.down(".results"+b.id);d.removeClassName("loading");d.down(".sourceContent").removeClassName("loading");(0<a.length||this.options.shownoresults)&&d.removeClassName("hidden");this.options.unifiedLoader&&!this.resultContainer.down(".results.loading")&&(this.options.loaderNode||this.fld).removeClassName("loading")}else d=this.resultContainer;
if(0==a.length&&!this.options.shownoresults)return!1;d.down("ul")&&d.down("ul").remove();this.container.select(".hide-button-wrapper").invoke("show");for(var e=new l.widgets.XList([],{icon:this.options.icon,classes:"suggestList",eventListeners:{click:function(k){c.setHighlightedValue(k);return!1},mouseover:function(){c.setHighlight(this.getElement())}}}),f=0,g=a.length;f<g;f++){var h=function(k){return((k||"")+"").escapeHTML()};h=(new Element("div")).insert((new Element("span",{"class":"suggestId"})).update(h(a[f].id))).insert((new Element("span",
{"class":"suggestValue"})).update(h(a[f].value))).insert((new Element("span",{"class":"suggestInfo"})).update(h(a[f].info))).insert((new Element("span",{"class":"suggestURL"})).update(h(a[f].url)));h=new l.widgets.XListItem(this.createItemDisplay(a[f],b),{containerClasses:"suggestItem "+(a[f].type||""),value:h,noHighlight:!0,containerTagName:"a"});h.containerElement.setAttribute("href",a[f].url||"javascript:void(0)");h.listItemElement.addEventListener("focusin",k=>c.setHighlight(k.currentTarget));
e.addItem(h)}0==a.length&&e.addItem(new l.widgets.XListItem(this.options.noresults,{classes:"noSuggestion",noHighlight:!0}));d.appendChild(e.getElement());this.suggest=d;c=this;0<this.options.timeout&&(this.toID=setTimeout(function(){c.clearSuggestions()},this.options.timeout))},createItemDisplay:function(a,b){var c=this.sInput?this.sInput.escapeHTML():this.sInput,d=((a.value||"")+"").escapeHTML();c=b.highlight?this.emphasizeMatches(c,d):d;a.hint&&(d=(a.hint+"").escapeHTML(),c+="<span class='hint'>"+
d+"</span>");this.options.displayValue?(c=(new Element("div")).insert((new Element("div",{"class":"value"})).update(c)),a.info&&(d=a.info+"",(void 0===b.resultInfoHTML?this.options.resultInfoHTML:b.resultInfoHTML)||(d=d.escapeHTML()),c.insert((new Element("div",{"class":"info"})).update("<span class='legend'>"+this.options.displayValueText+"</span>"+d)))):c=(new Element("span",{"class":"info"})).update(c);a.icon&&(a=0<=a.icon.indexOf(".")||0<=a.icon.indexOf("/")?new Element("img",{src:a.icon,"class":"icon"}):
new Element("i",{"class":"icon "+a.icon}),c.insert({top:a}));return c},emphasizeMatches:function(a,b){if(!a)return b;var c=b;a=a.split(/\s+/).uniq().compact();var d=0,e={};b=0;for(var f=a.length;b<f;b++)for(var g=c.toLowerCase().indexOf(a[b].toLowerCase());0<=g;){var h=c.substring(g,g+a[b].length),k="";a[b].length.times(function(){k+=" "});e[g]=h;c=c.substring(0,g)+k+c.substring(g+a[b].length);g=c.toLowerCase().indexOf(a[b].toLowerCase())}Object.keys(e).sortBy(function(m){return parseInt(m)}).each(function(m){var p=
c.substring(0,parseInt(m)+d),q=c.substring(parseInt(m)+e[m].length+d);c=p+"<em>"+e[m]+"</em>"+q;d+=9});return c},changeHighlight:function(a){var b=this.resultContainer;if(!b)return!1;if(this.iHighlighted)if(a==Event.KEY_DOWN){var c=this.iHighlighted.next();if(!c&&this.iHighlighted.up("div.results"))for(a=this.iHighlighted.up("div.results").next();a&&!c;)c=a.down("li"),a=a.next();c||=b.down("li")}else{if(a==Event.KEY_UP){c=this.iHighlighted.previous();if(!c&&this.iHighlighted.up("div.results"))for(a=
this.iHighlighted.up("div.results").previous();a&&!c;)c=a.down("li:last-child"),a=a.previous();c||=b.select("ul")[b.select("ul").length-1].down("li:last-child")}}else a==Event.KEY_DOWN?c=b.down("div.results")?b.down("div.results").down("li"):b.down("li"):a==Event.KEY_UP&&0<b.select("li")&&(c=b.select("li")[b.select("li").length-1]);c&&this.setHighlight(c)},setHighlight:function(a){this.iHighlighted&&this.clearHighlight();a.addClassName("xhighlight");this.iHighlighted=a;this.killTimeout()},clearHighlight:function(){this.iHighlighted&&
(this.iHighlighted.removeClassName("xhighlight"),delete this.iHighlighted)},highlightFirst:function(){if(this.suggest&&this.suggest.down("ul")){var a=this.suggest.down("ul").down("li");a&&this.setHighlight(a)}},hasActiveSelection:function(){return this.iHighlighted},setHighlightedValue:function(a){if(this.iHighlighted&&!this.iHighlighted.hasClassName("noSuggestion")){var b=function(e){return e.textContent||e.innerText},c=this.iHighlighted.down("img.icon");b={suggest:this,id:b(this.iHighlighted.down(".suggestId")),
value:b(this.iHighlighted.down(".suggestValue")),info:b(this.iHighlighted.down(".suggestInfo")),url:b(this.iHighlighted.down(".suggestURL")),icon:c?c.src:"",originalEvent:a};if(""==this.sInput&&""==this.fld.value)var d=c=b.value;else if(this.seps){a=-1;for(c=0;c<this.seps.length;c++)this.fld.value.lastIndexOf(this.seps.charAt(c))>a&&(a=this.fld.value.lastIndexOf(this.seps.charAt(c)));-1==a?d=c=b.value:(c=this.fld.value.substring(0,a+1)+b.value,d=c.substring(a+1))}else d=c=b.value;a=Event.fire(this.fld,
...
Event.stopObserving(this.fld,"keypress",this.onKeyPress),this.clearSuggestions(),this.fld.__x_suggest=null,this.fld.setAttribute("autocomplete","on"))}});return l}(XWiki||{});
