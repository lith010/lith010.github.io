...
this.form.up(".commentthread").previous(this.xcommentSelector).down("a.commentreply").show();a.up(this.xcommentSelector).next(".commentthread").insert({top:this.form});this.reloadEditor({callback:function(){this.getForm();this.addSubmitListener(this.form);this.addCancelListener();this.addPreview(this.form);this.form["XWiki.XWikiComments_replyto"].value=a.up(this.xcommentSelector)._x_number;this.form["XWiki.XWikiComments_comment"].focus();a.hide()}.bind(this)})}.bindAsEventListener(this))},addSubmitListener:function(a){a&&
!a._has_event&&(a._has_event=!0,a.down("input[type='submit']").observe("click",function(b){b.stop();document.fire("xwiki:actions:beforeSave");if(""!=a.down("textarea").value){b=new Hash(a.serialize(!0));b.set("xredirect",window.docgeturl+"?xpage=xpart&vm=commentsinline.vm&skin="+encodeURIComponent(c.skin));b.set("xpage","xpart");b.set("vm","commentsinline.vm");b.set("skin",c.skin);var d=$H(a.action.toQueryParams());b.keys().each(d.unset.bind(d));d=a.action.replace(/\?.*/,"?"+d.toQueryString());b.unset("action_cancel");
...
onComplete:function(e){a.enable();var f=a.down(".xwikicomment"),g="XWiki.XWikiComments_comment";void 0!==f&&this.hasCommentNumber(f)&&(g="XWiki.XWikiComments_"+this.extractCommentNumber(f)+"_comment");this.destroyEditor("[name='"+g+"']");this.requestSucceeded&&(this.container.update(e.responseText),e=$("submittedcomment"),f="",e?(f=e.value,e.remove(),this.formDisplayed=!1,this.displayHiddenForm(),this.reloadEditor({content:f,callback:function(){this.getForm();this.addSubmitListener(this.form);this.addCancelListener();
this.addPreview(this.form)}.bind(this)})):this.resetForm(),document.fire("xwiki:docextra:loaded",{id:"Comments",element:this.container}),this.container.fire("xwiki:captcha:reloaded"),a._x_notification.replace(new c.widgets.Notification("$services.localization.render('core.viewers.comments.add.done')","done")))}.bind(this)})}}.bindAsEventListener(this)))},addCancelListener:function(){this.form&&(this.initialLocation=new Element("span",{className:"hidden"}),$("_comments").insert(this.initialLocation),
this.form.down("a.cancel").observe("click",this.resetForm.bindAsEventListener(this)))},addPreview:function(a){if(a&&c.hasEdit&&"text"===a.down("input[name='defaultEditorId']").value){var b="$xwiki.getURL('__space__.__page__', 'preview')".replace("__space__",encodeURIComponent(c.currentSpace)).replace("__page__",encodeURIComponent(c.currentPage));a.commentElt=a.down("textarea");var d=a.down("input[type=submit]").up("div");a.previewButton=(new Element("span",{"class":"buttonwrapper"})).update(new Element("input",
