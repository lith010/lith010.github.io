...
$(this.options.insertAfterWhatNode||this.element);this.messageHolder=this.createMessageSpan();this.options.insertAfterWhatNode.up().appendChild(this.messageHolder);Object.extend(this,this.options);this.form&&(this.formObj=LiveValidationForm.getInstance(this.form),this.formObj.addField(this));this.cFocus=this.doOnFocus.bindAsEventListener(this);Event.observe(this.element,"focus",this.cFocus);if(!this.onlyOnSubmit)switch(this.elementType){case LiveValidation.CHECKBOX:this.cClick=this.validate.bindAsEventListener(this),
Event.observe(this.element,"click",this.cClick);case LiveValidation.SELECT:case LiveValidation.FILE:this.cChange=this.validate.bindAsEventListener(this);Event.observe(this.element,"change",this.cChange);break;default:this.onlyOnBlur||(this.cKeyup=this.deferValidation.bindAsEventListener(this),Event.observe(this.element,"keyup",this.cKeyup)),this.cBlur=this.validate.bindAsEventListener(this),Event.observe(this.element,"blur",this.cBlur)}},destroy:function(){this.formObj&&(this.formObj.removeField(this),
this.formObj.destroy());var b=this.element;Event.stopObserving(b,"focus",this.cFocus);if(!this.onlyOnSubmit)switch(this.elementType){case LiveValidation.CHECKBOX:Event.stopObserving(b,"click",this.cClick);case LiveValidation.SELECT:case LiveValidation.FILE:Event.stopObserving(b,"change",this.cChange);break;default:this.onlyOnBlur||Event.stopObserving(b,"keyup",this.cKeyup),Event.stopObserving(b,"blur",this.cBlur)}this.validations=[];this.removeMessageAndFieldClass()},add:function(b,a){this.validations.push({type:b,
params:a||{}});return this},remove:function(b,a){this.validations=this.validations.reject(function(c){return c.type==b&&c.params==a});return this},deferValidation:function(b){300<=this.wait&&this.removeMessageAndFieldClass();this.timeout&&clearTimeout(this.timeout);this.timeout=setTimeout(this.validate.bind(this),this.wait)},doOnBlur:function(){this.focused=!1;this.validate()},doOnFocus:function(){this.focused=!0;this.removeMessageAndFieldClass()},getElementType:function(){var b=this.element.nodeName.toUpperCase(),
a=this.element.type.toUpperCase();switch(!0){case "TEXTAREA"==b:return LiveValidation.TEXTAREA;case "INPUT"==b&&"TEXT"==a:return LiveValidation.TEXT;case "INPUT"==b&&"PASSWORD"==a:return LiveValidation.PASSWORD;case "INPUT"==b&&"CHECKBOX"==a:return LiveValidation.CHECKBOX;case "INPUT"==b&&"FILE"==a:return LiveValidation.FILE;case "SELECT"==b:return LiveValidation.SELECT;case "INPUT"==b:throw Error("LiveValidation::getElementType - Cannot use LiveValidation on an "+a.toLowerCase()+" input!");default:throw Error("LiveValidation::getElementType - Element must be an input, select, or textarea - "+
b.toLowerCase()+" was given!");}},doValidations:function(){this.validationFailed=!1;for(var b=0,a=this.validations.length;b<a;++b)if(this.validationFailed=!this.validateElement(this.validations[b].type,this.validations[b].params))return!1;this.message=this.validMessage;return!0},validateElement:function(b,a){switch(b){case Validate.Presence:case Validate.Confirmation:case Validate.Acceptance:this.displayMessageWhenEmpty=!0;break;case Validate.Custom:a.displayMessageWhenEmpty&&(this.displayMessageWhenEmpty=
!0)}var c=this.elementType==LiveValidation.SELECT?this.element.options[this.element.selectedIndex].value:this.element.value;if(b==Validate.Acceptance){if(this.elementType!=LiveValidation.CHECKBOX)throw Error("LiveValidation::validateElement - Element to validate acceptance must be a checkbox!");c=this.element.checked}var e=!0;try{b(c,a)}catch(d){if(d instanceof Validate.Error){if(""!==c||""===c&&this.displayMessageWhenEmpty)this.validationFailed=!0,this.message=d.message.split("\n")[0],e=!1}else throw d;
}finally{return e}},validate:function(){if(this.element.disabled)return!0;this.beforeValidation();var b=this.doValidations();b?(this.beforeValid(),this.onValid(),this.afterValid()):(this.beforeInvalid(),this.onInvalid(),this.afterInvalid());this.afterValidation();return b},enable:function(){this.element.disabled=!1;return this},disable:function(){this.element.disabled=!0;this.removeMessageAndFieldClass();return this},createMessageSpan:function(){var b=document.createElement("span");b.setAttribute("aria-live",
"polite");return b},insertMessageHolder:function(b){var a=this.validationFailed?this.invalidClass:this.validClass;$(b).addClassName(this.messageClass+" "+a);a=this.options.insertAfterWhatNode.up();var c=this.options.insertAfterWhatNode.next();c?a.insertBefore(b,c):a.appendChild(b)},insertMessage:function(){this.removeMessage();if(this.validationFailed||this.validMessage)if(this.displayMessageWhenEmpty&&(this.elementType==LiveValidation.CHECKBOX||""==this.element.value)||""!=this.element.value)this.messageHolder.addClassName(this.messageClass+
" "+(this.validationFailed?this.invalidClass:this.validClass)),this.messageHolder.textContent=this.message},addFieldClass:function(){this.removeFieldClass();if(this.validationFailed)this.element.hasClassName(this.invalidFieldClass)||this.element.addClassName(this.invalidFieldClass);else if(this.displayMessageWhenEmpty||""!=this.element.value)this.element.hasClassName(this.validFieldClass)||this.element.addClassName(this.validFieldClass)},removeMessage:function(){this.messageHolder.className="message-holder";
this.messageHolder.textContent=""},removeFieldClass:function(){this.element.removeClassName(this.invalidFieldClass);this.element.removeClassName(this.validFieldClass)},removeMessageAndFieldClass:function(){this.removeMessage();this.removeFieldClass()}};var LiveValidationForm=Class.create();
